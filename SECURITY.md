...
- **게시 경로**(`.github/workflows/release.yml`, T37): npm 게시는 `v*` 태그 push로만, GitHub 호스트 러너에서 `npm publish --provenance`로 이뤄집니다 — 게시된 tarball마다 "이 저장소의 이 커밋에서 이 워크플로가 빌드했다"는 Sigstore 증명이 npm에 함께 올라가고 `npm audit signatures`로 검증할 수 있습니다. 게시 직전 `prepublishOnly`가 전체 검사 + tarball 기준 audit(fail-closed)을 다시 돌립니다. `id-token: write`는 그 job에만 부여되고, 첫 게시에 쓰는 npm 토큰은 저장소 secret에만 있으며 첫 게시 뒤 trusted publishing(OIDC, 장기 토큰 없음)으로 전환합니다.
