...
첫 공개 버전은 `0.1.0`(2026-09-04)입니다. 0.x 단계에서는 **가장 최근 minor 라인**(현재 `0.1.x`)과 `main` 브랜치 최신 커밋만 보안 패치 대상입니다. `1.0.0` 이후에는 이 절을 최신 major 기준으로 갱신합니다.

| 버전 | 지원 여부 |
|---|---|
| `main`(미게시 최신) | ✅ |
| `0.1.x`(최신 게시 라인) | ✅ |
| 그 이전 0.x 라인 | ❌ — 최신 라인으로 업그레이드 |
...
- **게시 경로**(`.github/workflows/release.yml`, T37): npm 게시는 `v*` 태그 push로만, GitHub 호스트 러너에서 `npm publish --provenance`로 이뤄집니다 — 게시된 tarball마다 "이 저장소의 이 커밋에서 이 워크플로가 빌드했다"는 Sigstore 증명이 npm에 함께 올라가고 `npm audit signatures`로 검증할 수 있습니다. 게시 직전 `prepublishOnly`가 전체 검사 + tarball 기준 audit(fail-closed)을 다시 돌립니다. `id-token: write`는 그 job에만 부여됩니다. 첫 게시(`0.1.0`)에만 npm granular token을 저장소 secret으로 썼고, 그 뒤 npm trusted publishing(OIDC — 워크플로 실행마다 GitHub id-token을 npm의 단기 토큰으로 교환)으로 전환해 secret과 토큰을 모두 폐기했습니다. 즉 이 저장소에는 npm에 게시할 수 있는 장기 자격증명이 없고, 게시는 `Trapa-Eureka/retail-mcp`의 `release.yml`이 `npm-publish` environment(사람 승인 필요)에서 실행될 때만 가능합니다.
