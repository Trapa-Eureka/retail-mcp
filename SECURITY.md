...
- **CI 보안 게이트**(`.github/workflows/ci.yml`, TASKS T35): 매 push/PR에서 dependency audit(lockfile 기준, 승인되지 않은 새 취약점은 fail-closed로 막음), 커밋된 시크릿 패턴 스캔, SBOM(CycloneDX) 생성, 게시 tarball 기준 audit(`npm run verify:pack`)을 자동으로 돕니다 — `src/adapters/auditLockfile.ts`/`src/core/secretScan.ts` 참고.
