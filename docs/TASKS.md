...
- 완료 기준: [ ] 008의 8단계 release gate 전부 통과 [ ] tarball fresh install 후 CLI/MCP 실행 확인 [ ] `npm view`로 패키지명 재사용 가능 여부 최종 확인(REL-008, 사람 확인 필요 항목) [ ] **main 브랜치 ruleset이 살아 있는지 사람 확인**(2차 적대적 검수 SR2-CI-004, 2026-09-04 생성 — ruleset id `22244613`, `gh api repos/Trapa-Eureka/retail-mcp/rules/branches/main --jq '[.[].type]'`가 `["deletion","non_fast_forward","pull_request","required_status_checks"]`를 돌려주고, required checks 7개(test ×4 OS/Node matrix, coverage, postgres component, dependency audit + secret scan)·bypass actor 0명·enforcement `active`인지 확인. 릴리스 직전에 누가 비활성화하지 않았는지 보는 항목이다) [ ] `npm run check` 통과 [ ] 사용자에게 npm publish 준비 단계 진입 여부 확인
