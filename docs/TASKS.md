...
### T35 — 테스트 게이트 P1 (QA-002~006, OPS-006 CI matrix 포함) · 상태: TODO · 의존: T34
...
- 완료 기준: [ ] release gate에 `npm run coverage` 포함 + 위험 모듈 threshold 추가 [ ] CI에서 실 Postgres 서비스로 migration/READ ONLY/advisory lock/explore_sql timeout 컴포넌트 테스트 [ ] CI matrix에 최소 지원 OS/Node LTS로 `npm run verify:pack`(clean tarball install) 포함(OPS-006) [ ] 005~007 재검수 체크리스트 항목이 전부 자동 테스트로 연결됐는지 대조표 작성 [ ] `npm audit`/allowlist assertion을 release 워크플로에 연결 [ ] `npm run check` 통과
