...
- 완료 기준: [ ] export한 파일을 T15 스키마로 다시 파싱하면 원 데이터와 일치(왕복 테스트) [ ] 매장명 포함 확인 [ ] check 통과

### T20 — 본사 통합 모드 · 상태: TODO · 의존: T18, T16
...
- 완료 기준: [ ] 지점 2개 스냅샷을 한 수집 폴더에 넣고 매장명으로 필터링해 통합 조회하는 테스트(§5 "본점만" 예시와 동일 동작) [ ] 한 지점 스냅샷이 파싱 중간에 실패해도 다른 지점 데이터·watermark에 영향 없음 [ ] check 통과

### T21 — 온보딩 CLI · 상태: TODO · 의존: T14, T20
...
- 완료 기준: [ ] 비대화식(플래그 또는 스크립트 입력) 실행으로 설정 파일 생성 테스트 [ ] 필수 값 누락 시 재질문/명확한 에러 [ ] 생성된 설정으로 T18 스크립트가 그대로 기동 [ ] check 통과

### T22 — e2e + 문서 갱신 · 상태: TODO · 의존: T21
- 목표: 지점 단독 시나리오(파일→파싱→적재→알림)와 본사 통합 시나리오(지점 2곳 스냅샷→통합 조회) e2e, README/SPEC §12에 실제 사용 절차 반영.
- 완료 기준: [ ] 두 e2e 시나리오 통과 [ ] README에 CSV/Excel 채널 퀵스타트 추가 [ ] check 통과
