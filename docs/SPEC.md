...
| **v0.2** | **CSV/Excel 업로드 기반 재고 데이터소스(다음 실제 출시 목표)** — 폴더 감시 채널, 임베디드 PGlite 웨어하우스(§12) | 실행 모델(§8) 확정 |
...
- [ ] **실행 모델**: 폴더 감시를 기존 `agent:reorder`와 같은 주기 스캔(cron)으로 할지, 상시 워처(파일시스템 이벤트 감지) 데몬으로 할지 — §12는 주기 스캔을 권장안으로 적되 확정은 아니다.
- [ ] **다지점 헤드오피스 통합 조회**: 지점마다 로컬 PGlite면 지점별로 데이터가 분리된다. 본사가 여러 지점을 한 번에 보려면 지점 결과를 어떻게 모을지(지점별 산출물 파일을 본사가 취합 / 지점이 공용 Neon에 업로드 / 기타) 결정 필요 — v0.2 최소 범위(단일 지점)에는 급하지 않지만 "다지점" 제품 전제와 직결된다.
...
실행 모델(주기 스캔 vs 상시 워처), 다지점 헤드오피스 통합 조회 방법, PGlite 동시 접근 검증 — 세 가지는 아직 결정되지 않았다. §8 참고.
