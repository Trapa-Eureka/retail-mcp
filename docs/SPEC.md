...
> ⚠️ **2026-09-03 재검토**: 위 전제("Loyverse류 POS를 이미 쓴다")는 확정된 파일럿 고객이 아니라 가정이다. POS 기기 자체는 관찰됐지만 브랜드(=API 존재 여부)는 미확인이고, 실제로는 Excel/자체 ERP/수기 재고 관리가 더 흔할 수 있다는 신호가 있다 — §8 미결 사항의 최우선 항목 참고. v0.1의 Loyverse 선택은 "코어 로직(셀스루·재주문 계산)을 하나의 깨끗한 데이터소스로 먼저 검증한다"는 MVP 순서 판단이었고, 데이터소스는 `LoyverseClient` 인터페이스 뒤에 있어 CSV/Excel 등으로 교체·추가해도 core/ETL 오케스트레이션/MCP 도구/에이전트는 그대로 재사용된다(DESIGN §1 어댑터 구조) — 다만 **어느 데이터소스를 다음에 만들지는 실제 파일럿 후보의 재고 관리 방식이 확인돼야 결정할 수 있다.**
...
- [ ] **(최우선, 2026-09-03 제기)** 파일럿 후보의 실제 재고 관리 방식 확인 — Loyverse류 API형 POS인지, Excel/자체 ERP/수기 관리인지에 따라 v0.1 데이터소스 전략(Loyverse 어댑터 유지 vs CSV/Excel 어댑터 우선 개발)이 갈린다. §11 참고. **이 항목이 풀리기 전까지 특정 데이터소스에 추가 투자하지 않는다.**
- [ ] Loyverse 원가(cost) 필드 데이터 품질 → 마진 지표 v0.2 포함 여부 (Loyverse를 실제로 쓰는 파일럿이 확정된 경우에만 유효)
- [ ] 리드타임을 공급자별로 다르게 줄 방법 (v0.1은 전역 기본값 + 인자)
- [ ] 재주문 리포트 수신자: 오너 단일 vs 지점장별 분리 (v0.1은 단일 수신자)
- [ ] 파일럿 매장 후보 확정 (지점 2개 이상 — POS/데이터소스 종류는 위 최우선 항목으로 먼저 확인)
