...
**Migration checksum normalisation and 0.1.0 re-baseline (2026-09-04)** — `computeChecksum()` in `src/adapters/migrationRunner.ts` hashes `normalizeMigrationSql(sql)` rather than the raw file: full-line `--` comments and blank lines are dropped and trailing whitespace trimmed, so translating or rewording a comment in an already-applied migration no longer trips the "content changed" guard, while every statement, identifier and literal is still hashed byte-for-byte. Because 0.1.0 recorded raw-file sha256 values (with Korean comments) in `schema_migrations`, `LEGACY_RAW_CHECKSUMS` pins those eight values; `runMigrations` accepts exactly that recorded value once per id, rewrites it to the current normalised checksum (reported in `RunMigrationsResult.rebaselined`) and moves on — any other mismatch still aborts, and no user action is needed.
