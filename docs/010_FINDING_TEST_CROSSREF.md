...
| DOC-004 | npm 배포 사용법 문서 없음 | 예정(T36) |
| DOC-005 | 001~003 문서의 lifecycle 표기 없음 | 해소(T28) — `docs/001~003`에 상태 라벨 추가 |

> 참고: `docs/009` 6번째 줄의 요약은 "DOC-002~004는 T36에서"라고 적혀 있어 위 표(DOC-002/003은 T28에서
> 이미 실질적으로 해소, T36은 DOC-004/005 잔여분만 — `docs/TASKS.md` T36 절의 공식 범위)와 살짝
> 어긋난다. 이건 009 자체의 요약 문장이 T28 완료 후 갱신되지 않은 것으로 보인다 — T36이 문서
> 정합성 담당이니 그때 함께 바로잡는다(지금 T35 범위 밖이라 여기서는 대조표만 정확히 반영했다).
