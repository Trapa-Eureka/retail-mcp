...
## DATA-001 — snapshot export loses `포장수량` (pack size)

- Severity: **Critical**
- Area: `src/core/snapshotExport.ts`
- Reproduction: Exporting a ProductRow with `packSize: "24"` yields no `포장수량` anywhere in the header or the rows.
...
- Fix criteria: Include `포장수량` in the snapshot schema/header/row and add a regression test that packSize is identical after `parse → export → parse`.
- **Resolution (T31)**: Added `포장수량` to `COLUMNS`/the row mapping in `core/snapshotExport.ts`. Re-verified by including raw rows with `포장수량` mixed in within the round-trip test.
