...
리테일 다지점용 셀스루·재고 BI MCP 서버 + 재주문 제안 에이전트. v0.1 데이터 소스는 Loyverse 단일. 배경·지표 정의는 `docs/SPEC.md`, 구현 설계는 `docs/DESIGN.md`.

## 스택

- Node.js 20+, TypeScript **strict** (`noUncheckedIndexedAccess` 포함)
- MCP: `@modelcontextprotocol/sdk` — stdio transport
- 웨어하우스: Postgres (운영 = Neon/Supabase, `pg` 드라이버) / 테스트 = **PGlite** (인프로세스 Postgres, 네트워크 0)
- POS 소스: Loyverse REST API (`LOYVERSE_API_TOKEN`)
...
```
src/
  core/        # 순수 로직: metrics(셀스루·커버일수·재주문량), etl 변환, 타입 — 외부 IO 없음
  etl/         # 동기화 오케스트레이션 (LoyverseClient + Warehouse 조립, 커서 관리)
  adapters/    # loyverseClient, pgWarehouse, resendProvider
  mocks/       # FixtureLoyverseClient, PGlite 웨어하우스 헬퍼, MockNotificationProvider, FixedClock
  agent/       # reorder.ts — 스케줄 실행 진입점 (얇은 오케스트레이션만)
  server.ts    # MCP 서버 진입점 (도구 등록·조립만, 로직 없음)
migrations/    # 001_init.sql ... 순번 SQL 파일
tests/  fixtures/loyverse/  scripts/
```
...
- MCP 질의 도구의 SQL은 **파라미터라이즈드 고정 쿼리만**. 자유 SQL 도구는 v0.1에 없다 (v0.2 대기열).
...
4. 웨어하우스 **쓰기는 ETL 경로만**. MCP 질의 도구는 읽기 전용 (운영 DB에는 읽기 전용 롤 사용).
