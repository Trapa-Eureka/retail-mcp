...
> 🚫 **npm publish 아직 미실행(2026-09-03 출시 차단 → 2026-09-04 기계적 gate 통과)**: npm 배포 전 적대적 검수(`docs/004~008` 40건 + 2차 `docs/010` 19건)는 전부 처리됐고 T37의 8단계 release gate 중 기계적 단계(1~7)도 통과했다. 남은 것은 사람 확인 항목(npm scope 소유권, provenance 방식)과 사용자의 게시 승인이다 — `docs/TASKS.md` T37 체크리스트 참고. 이 배너는 실제 `npm publish` 직후 제거한다.
