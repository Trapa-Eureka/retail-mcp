...
> ⚠️ **사용처 재검토 중(2026-09-03)**: v0.1은 POS 데이터소스로 Loyverse를, 조회 채널로 Claude Code(개발자/운영자용)를 전제한다 — 둘 다 확정된 파일럿 고객 기준이 아니라 가정이다. 실제 타겟(불특정 다수 리테일 다지점)이 Excel/자체 ERP/수기로 재고를 관리할 가능성이 있고, 오너는 MCP를 직접 쓰기보다 에이전트의 이메일 리포트만 받는 쪽이 현실적이다. 상세와 다음 결정 기준은 `docs/SPEC.md` §11 참고.
